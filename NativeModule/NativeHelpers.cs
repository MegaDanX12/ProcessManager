...
using System.Text;
using static NativeModule.NativeMethods;
...
namespace NativeModule
{
    ...
    public static class NativeHelpers
    {
        ...
        /// <summary>
        /// Delegato per l'elaborazione di un evento.
        /// </summary>
        ...
        /// <summary>
        /// Dizionario che associa il nome DOS di un'unità alla sua lettera.
        /// </summary>
        public static readonly ConcurrentDictionary<string, string> DosNamesMapping = new();
        ...
        public static bool StartProcess(string FilePath)
        {
            ...
            else
            {
                ...
                LogEntry Entry = BuildLogEntryForWin32Error("Non è stato possibile creare un processo", EventAction.OtherActions, null, ex.NativeErrorCode, ex.Message);
        ...
        /// </summary>
        /// <param name="Handle">Handle al processo.</param>
        /// <returns>Uno dei valori di <see cref="Models.ProcessInfo.ProcessPriority"/>, 0 in caso di errore.</returns>
        ...
        public static string GetProcessName(SafeProcessHandle Handle, uint? PID = null, bool DebugLoggingCall = false)
        {
            if (Handle != null)
            {
                if (!PID.HasValue)
                ...
                switch (PID)
                {
                    ...
                    case 116:
                        return "Registry";
                    default:
                        string FullPath = GetProcessFullPathNT(Handle, DebugLoggingCall);
            ...
            if (ProcessHandle != IntPtr.Zero)
            {
                ...
                if (ParentStartTime.HasValue)
                {
                    ...
                    else
                    {
                        ...
                        Logger.WriteEntry(Entry);
                        Win32OtherFunctions.CloseHandle(ProcessHandle);
                        return string.Empty;
                    }
                }
                else
        ...
        /// </summary>
        /// <param name="Handle">Handle al processo.</param>
        /// <returns>Il percorso completo dell'eseguibile del processo, "Non disponibile" in caso di errore.</returns>
        public static string GetProcessFullPath(SafeProcessHandle Handle, bool DebugLoggingCall = false)
        {
            if (Handle != null && !Handle.IsInvalid)
            {
                StringBuilder ProcessPath = new((int)Win32Constants.MAX_PATH);
                uint PathLength = Win32ProcessFunctions.GetProcessImageFileName(Handle.DangerousGetHandle(), ProcessPath, Win32Constants.MAX_PATH);
                if (PathLength != 0)
                {
                    string ProcessPathString = ProcessPath.ToString();
                    foreach (KeyValuePair<string, string> pair in DosNamesMapping)
                    {
                        if (ProcessPathString.Contains(pair.Key))
                        {
                            ProcessPathString = ProcessPathString.Replace(pair.Key, pair.Value);
                        }
                    }
                ...
                {
                    Win32Exception ex = new(Marshal.GetLastWin32Error());
                    LogEntry Entry = BuildLogEntryForWin32Error("Non è stato possibile recuperare il percorso completo di un processo", EventAction.ProcessPropertiesRead, null, ex.NativeErrorCode, ex.Message);
                    Logger.WriteEntry(Entry);
                    return Properties.Resources.UnavailableText;
                }
            }
        ...
        public static string GetProcessFullPathNT(SafeProcessHandle Handle, bool DebugLoggingCall = false)
        {
            if (Handle != null && !Handle.IsInvalid)
            {
        ...
        private static FileVersionInfo GetFileVersionInfo2(SafeProcessHandle Handle, string ProcessFullPath = null)
        {
            if (Handle != null && !Handle.IsInvalid)
            {
                try
                {
                    ...
                    {
                        LogEntry Entry = BuildLogEntryForWarning("Non è stato possibile recuperare informazioni sulla versione di un file, percorso file non disponibile", EventAction.ProcessPropertiesRead, Handle, ProcessName: Path.GetFileName(ProcessFullPath));
                        Logger.WriteEntry(Entry);
                        return null;
                    }
                }
                    ...
                    for (uint i = 0; i < NumberOfHandles; i++)
                    {
                        ...
                        if (ProcessID != 0)
                        {
                            if ((uint)HandleInfo.ProcessID.ToInt32() == ProcessID)
                            {
                                ...
                                if (Result == Win32Constants.STATUS_SUCCESS)
                                {
                                    IntPtr TypeInfoStructurePointer = Marshal.AllocHGlobal(Marshal.SizeOf(typeof(Win32Structures.OBJECT_TYPE_INFORMATION)));
                                    ...
                                    if (Result == Win32Constants.STATUS_SUCCESS)
                                    {
                                        ...
                                        HandleInfo Info = new HandleInfo(HandleType, HandleName, HandleValue, ObjectAddress, GrantedAccess, ReferencesCount, HandlesCount, PagedPoolUsage, NonPagedPoolUsage);
        ...
        public static ThreadInfo GetThreadInfo(uint TID, uint PID)
        {
            ...
            string BasePriority = GetThreadBasePriority(TID, PID).ToString("N0", CultureInfo.InvariantCulture);
        ...
        private static uint GetThreadBasePriority(uint TID, uint PID)
        {
        ...
        public static List<MemoryRegionInfo> GetProcessMemoryInfo(SafeProcessHandle Handle)
        {
            ...
            if (Is32BitProcess.HasValue)
            {
                ...
                if (Is32BitProcess.Value)
                {
                    ...
                    {
                        if (Win32MemoryFunctions.VirtualQueryEx32(Handle.DangerousGetHandle(), Address, out Win32Structures.MEMORY_BASIC_INFORMATION Buffer, (ulong)Marshal.SizeOf(typeof(Win32Structures.MEMORY_BASIC_INFORMATION))) == IntPtr.Zero)
                        {
                            Win32Exception ex = new(Marshal.GetLastWin32Error());
                            LogEntry Entry;
                ...
                else
                {
                    while (Address.ToInt64() < SystemInfo.MaximumApplicationAddress.ToInt64())
                    {
                        if (Win32MemoryFunctions.VirtualQueryEx64(Handle.DangerousGetHandle(), Address, out Win32Structures.MEMORY_BASIC_INFORMATION64 Buffer, (ulong)Marshal.SizeOf(typeof(Win32Structures.MEMORY_BASIC_INFORMATION64))) == IntPtr.Zero)
                        {
                            Win32Exception ex = new(Marshal.GetLastWin32Error());
                            LogEntry Entry;
                            if (ex.NativeErrorCode == Win32Constants.ERROR_INVALID_PARAMETER)
        ...
        /// <returns>La stringa aggiuntiva, oppure una stringa vuota se non viene trovat</returns>
        private static string GetCombinedProtectionValues(Win32Enumerations.MemoryProtections Protection)
        {
            ...
            {
                if (value == Win32Enumerations.MemoryProtections.PAGE_GUARD || value == Win32Enumerations.MemoryProtections.PAGE_NOCACHE || value == Win32Enumerations.MemoryProtections.PAGE_WRITECOMBINE || value == Win32Enumerations.MemoryProtections.PAGE_ENCLAVE_THREAD_CONTROL || value == Win32Enumerations.MemoryProtections.PAGE_ENCLAVE_UNVALIDATED)
                {
        ...
        public static bool? IsProcess32Bit(SafeProcessHandle Handle)
        {
            ...
            {
                Win32Exception ex = new(Marshal.GetLastWin32Error());
                LogEntry Entry = BuildLogEntryForWin32Error("Non è stato possibile determinare se un processo appartiene è a 32 bit", EventAction.ProcessPropertiesRead, Handle, ex.NativeErrorCode, ex.Message);
                Logger.WriteEntry(Entry);
                return null;
            }
        }
            ...
            if (ProcessHandle != null && !ProcessHandle.IsInvalid)
            {
                ...
                if (!Win32OtherFunctions.CompareObjectHandles(CurrentProcessHandle, ProcessHandle.DangerousGetHandle()))
                {
                    if (Info.PagesState != Enum.GetName(typeof(Win32Enumerations.MemoryPageState), Win32Enumerations.MemoryPageState.MEM_COMMIT))
                    {
                        ...
                        if (IntPtr.Size == 4)
                        {
                            Result = Win32MemoryFunctions.VirtualProtectEx(ProcessHandle.DangerousGetHandle(), new IntPtr(Convert.ToInt32(Info.BaseAddress, 16)), new IntPtr(Convert.ToInt32(Info.Size)), NewProtectionValue, out Win32Enumerations.MemoryProtections OldValue);
                            if (!Result)
                            {
                                Win32Exception ex = new(Marshal.GetLastWin32Error());
                        ...
                        {
                            Result = Win32MemoryFunctions.VirtualProtectEx(ProcessHandle.DangerousGetHandle(), new IntPtr(Convert.ToInt64(Info.BaseAddress, 16)), new IntPtr(Convert.ToInt32(Info.Size)), NewProtectionValue, out Win32Enumerations.MemoryProtections OldValue);
                            if (!Result)
                            {
                                Win32Exception ex = new(Marshal.GetLastWin32Error());
                                LogEntry Entry = BuildLogEntryForWin32Error("Non è stato possibile cambiare la protezione di una regione di memoria", EventAction.MemoryInfoManipulation, ProcessHandle, ex.NativeErrorCode, ex.Message);
            ...
            if (ProcessHandle != null && !ProcessHandle.IsInvalid)
            {
                ...
                if (!Win32OtherFunctions.CompareObjectHandles(CurrentProcessHandle, ProcessHandle.DangerousGetHandle()))
                {
                    if (Info.PagesState != "MEM_FREE")
                    {
                        if (IntPtr.Size == 4)
                        {
                            bool Result = Win32MemoryFunctions.VirtualFreeEx(ProcessHandle.DangerousGetHandle(), new IntPtr(Convert.ToInt32(Info.BaseAddress, 16)), IntPtr.Zero, Win32Enumerations.FreeOperationType.MEM_RELEASE);
                            if (!Result)
                            {
                                Win32Exception ex = new(Marshal.GetLastWin32Error());
                        ...
                        {
                            bool Result = Win32MemoryFunctions.VirtualFreeEx(ProcessHandle.DangerousGetHandle(), new IntPtr(Convert.ToInt64(Info.BaseAddress, 16)), IntPtr.Zero, Win32Enumerations.FreeOperationType.MEM_RELEASE);
                            if (!Result)
                            {
                                Win32Exception ex = new(Marshal.GetLastWin32Error());
                                LogEntry Entry = BuildLogEntryForWin32Error("Non è stato possibile liberare una regione di memoria", EventAction.MemoryInfoManipulation, ProcessHandle, ex.NativeErrorCode, ex.Message);
            ...
            if (ProcessHandle != null && !ProcessHandle.IsInvalid)
            {
                ...
                if (!Win32OtherFunctions.CompareObjectHandles(CurrentProcessHandle, ProcessHandle.DangerousGetHandle()))
                {
                    if (Info.PagesState != "MEM_FREE")
                    {
                        if (IntPtr.Size == 4)
                        {
                            bool Result = Win32MemoryFunctions.VirtualFreeEx(ProcessHandle.DangerousGetHandle(), new IntPtr(Convert.ToInt32(Info.BaseAddress, 16)), IntPtr.Zero, Win32Enumerations.FreeOperationType.MEM_DECOMMIT);
                            if (!Result)
                            {
                                Win32Exception ex = new(Marshal.GetLastWin32Error());
                        ...
                        {
                            bool Result = Win32MemoryFunctions.VirtualFreeEx(ProcessHandle.DangerousGetHandle(), new IntPtr(Convert.ToInt64(Info.BaseAddress, 16)), IntPtr.Zero, Win32Enumerations.FreeOperationType.MEM_DECOMMIT);
                            if (!Result)
                            {
                                Win32Exception ex = new(Marshal.GetLastWin32Error());
                                LogEntry Entry = BuildLogEntryForWin32Error("Non è stato possibile annullare la mappatura una regione di memoria", EventAction.MemoryInfoManipulation, ProcessHandle, ex.NativeErrorCode, ex.Message);
        ...
        public static bool EmptyProcessWorkingSet(SafeProcessHandle ProcessHandle)
        {
            if (ProcessHandle != null && !ProcessHandle.IsInvalid)
            {
                if (Settings.SafeMode)
                {
                    ...
                    else
                    {
                        ...
                        {
                            //Se il processo a cui l'handle fa riferimento è un processo di sistema, non viene eseguita alcuna operazione e l'operazione è considerata fallita.
                            LogEntry Entry = BuildLogEntryForWarning("Non è stato possibile liberare il working set di un processo, azioni su processi di sistema non sono permesse", EventAction.MemoryInfoManipulation, ProcessHandle);
                            Logger.WriteEntry(Entry);
                            return false;
                        }
                    }
                ...
                if (!Win32OtherFunctions.CompareObjectHandles(CurrentProcessHandle, ProcessHandle.DangerousGetHandle()))
                {
                    ...
                    {
                        Win32Exception ex = new(Marshal.GetLastWin32Error());
                        LogEntry Entry = BuildLogEntryForWin32Error("Non è stato possibile liberare il working set di un processo", EventAction.MemoryInfoManipulation, ProcessHandle, ex.NativeErrorCode, ex.Message);
                        Logger.WriteEntry(Entry);
                        return false;
                    }
                }
                ...
                {
                    LogEntry Entry = BuildLogEntryForWarning("Non è stato possibile liberare il working set di un processo, azioni sul processo corrente non sono permesse", EventAction.MemoryInfoManipulation);
                    Logger.WriteEntry(Entry);
                    return false;
                }
            }
            ...
            {
                LogEntry Entry = BuildLogEntryForWarning("Non è stato possibile liberare il working set di un processo, handle al processo non valido", EventAction.MemoryInfoManipulation, null);
                Logger.WriteEntry(Entry);
                return false;
            }
        }
        ...
        #endregion
        #region Drives Monitoring Methods
        /// <summary>
        /// Recupera le lettere di unità attualmente in uso.
        /// </summary>
        public static void GetAvailableDrives()
        {
            uint BufferSize = Win32OtherFunctions.GetLogicalDriveStrings(0, null);
            char[] Buffer;
            uint Result;
            string FullDriveList;
            string Drive;
            string DriveDOSName1;
            string DriveDOSName2;
            if (BufferSize != 0)
            {
                Buffer = new char[BufferSize];
                Result = Win32OtherFunctions.GetLogicalDriveStrings(BufferSize, Buffer);
                if (Result != 0)
                {
                    FullDriveList = new string(Buffer);
                    string[] Drives = FullDriveList.Split('\0');
                    Buffer = new char[100];
                    foreach (string drive in Drives)
                    {
                        if (drive != string.Empty)
                        {
                            ...
                            Result = Win32OtherFunctions.QueryDosDevice(Drive, Buffer, 100);
                            if (Result != 0)
                            {
                                DriveDOSName1 = new string(Buffer);
                                DriveDOSName2 = DriveDOSName1.Remove(DriveDOSName1.IndexOf('\0'));
                                _ = DosNamesMapping.TryAdd(DriveDOSName2, Drive);
                            }
                            else
                            {
                                Win32Exception ex = new(Marshal.GetLastWin32Error());
                                LogEntry Entry = BuildLogEntryForWin32Error("Non è stato possibile recuperare il nome DOS di uno dei dischi connessi al sistema", EventAction.ProgramStartup, null, ex.NativeErrorCode, ex.Message);
                                Logger.WriteEntry(Entry);
                            }
                        }
                    }
                ...
                {
                    Win32Exception ex = new(Marshal.GetLastWin32Error());
                    LogEntry Entry = BuildLogEntryForWin32Error("Non è stato possibile recuperare le lettere di unità dei dischi connessi al sistema", EventAction.ProgramStartup, null, ex.NativeErrorCode, ex.Message);
                    Logger.WriteEntry(Entry);
                }
            }
            else
        ...
        /// <summary>
        /// Aggiunge la lettera di unità di un disco appena connesso.
        /// </summary>
        /// <param name="Letter">Lettera assegnata all'unità.</param>
        public static void AddDriveLetter(string Letter)
        {
            string DriveDOSName1;
            string DriveDOSName2;
            char[] Buffer = new char[100];
            uint Result = Win32OtherFunctions.QueryDosDevice(Letter, Buffer, 100);
            if (Result != 0)
            {
                DriveDOSName1 = new string(Buffer);
                DriveDOSName2 = DriveDOSName1.Remove(DriveDOSName1.IndexOf('\0'));
                _ = DosNamesMapping.TryAdd(DriveDOSName2, Letter);
            }
            else
            {
                Win32Exception ex = new(Marshal.GetLastWin32Error());
                LogEntry Entry = BuildLogEntryForWin32Error("Non è stato possibile recuperare il nome DOS di uno dei dischi connessi al sistema", EventAction.OtherActions, null, ex.NativeErrorCode, ex.Message);
                Logger.WriteEntry(Entry);
            }
        }

        /// <summary>
        /// Rimuove la lettera di unità di un disco appena disconnesso.
        /// </summary>
        /// <param name="Letter">Lettera che era assegnata all'unità.</param>
        public static void RemoveDriveLetter(string Letter)
        {
            string DriveDOSName1;
            string DriveDOSName2;
            char[] Buffer = new char[100];
            ...
            if (Result != 0)
            {
                DriveDOSName1 = new string(Buffer);
                DriveDOSName2 = DriveDOSName1.Remove(DriveDOSName1.IndexOf('\0'));
                _ = DosNamesMapping.TryRemove(DriveDOSName2, out _);
            }
            else
            {
                Win32Exception ex = new(Marshal.GetLastWin32Error());
        ...
        /// </summary>
        /// <param name="WindowEventProcedure">Puntatore alla funzione che gestirà gli eventi.</param>
                ...
                if (Is32BitProcess.HasValue)
                {
                    ...
                    if (br.ReadUInt32() == 0x00004550)
                    {
                        ...
                        if (Is32BitProcess.Value)
                        {
                            ...
                            fs.Seek(8 * OptionalHeaderWindowsSpecific32.NumberOfRvaAndSizes, SeekOrigin.Current);
                        ...
                        else
                        {
                            ...
                            fs.Seek(8 * OptionalHeaderWindowsSpecific64.NumberOfRvaAndSizes, SeekOrigin.Current);
                            SectionHeaders = GetSectionHeaders(br, FileHeader.NumberOfSections);
                            foreach (SectionHeader header in SectionHeaders)
                            {
                                SectionInfo = new(header.Name, (IntPtr)header.VirtualAddress, header.VirtualSize);
                ...
                if (FileMappingHandle != IntPtr.Zero)
                {
                    ...
                    if (MappedViewBase != IntPtr.Zero)
                    {
                        if (Win32FileFunctions.GetFileSize(FileHandle, out long FileSize))
                        {
                            ...
                            {
                                Win32FileFunctions.UnmapViewOfFile(MappedViewBase);
                                Win32OtherFunctions.CloseHandle(FileMappingHandle);
                                Win32OtherFunctions.CloseHandle(FileHandle);
        ...
        private static object[] GetDataForLogging(SafeProcessHandle Handle, bool OnlyPID, bool OnlyProcessName, uint PID = 0, string ProcessName = null, bool DebugLoggingCall = false)
        {
            object[] Data = new object[2];
            if (OnlyPID)
            {
                PID = GetProcessPID(Handle, DebugLoggingCall);
            }
            else if (OnlyProcessName)
            {
                ProcessName = GetProcessName(Handle, PID, DebugLoggingCall);
            }
            else
            {
                PID = GetProcessPID(Handle, DebugLoggingCall);
