...
{
    /// <summary>
    /// Logica di interazione per ProcessPropertiesWindow.xaml
    /// </summary>
    public partial class ProcessPropertiesWindow : Window
    {
        public ProcessPropertiesWindow(SafeProcessHandle Handle, Dictionary<string, string> EnabledSettings, ProcessPropertiesVM VM)
        {
            DataContext = VM;
            InitializeComponent();
        ...
        {
            ProcessPropertiesVM DataContext = (ProcessPropertiesVM)this.DataContext;
            switch (CategoriesComboBox.SelectedIndex)
            {
                case 0:
                    CountersDataGrid.ItemsSource = DataContext.NetPerformanceInfo.ExceptionsInfo?.Values;
                    break;
                case 1:
                    CountersDataGrid.ItemsSource = DataContext.NetPerformanceInfo.InteropInfo?.Values;
                    break;
                case 2:
                    CountersDataGrid.ItemsSource = DataContext.NetPerformanceInfo.JitInfo?.Values;
                    break;
                case 3:
                    if (DataContext.NetPerformanceInfo.LoadingInfo is not null)
                    {
                        CountersDataGrid.ItemsSource = DataContext.NetPerformanceInfo.LoadingInfo.Values;
                    }
                    break;
                case 4:
                    CountersDataGrid.ItemsSource = DataContext.NetPerformanceInfo.LockAndThreadInfo?.Values;
                    break;
                case 5:
                    CountersDataGrid.ItemsSource = DataContext.NetPerformanceInfo.MemoryInfo?.Values;
                    break;
                case 6:
                    CountersDataGrid.ItemsSource = DataContext.NetPerformanceInfo.NetworkingInfo?.Values;
                    break;
                case 7:
                    CountersDataGrid.ItemsSource = DataContext.NetPerformanceInfo.SecurityInfo?.Values;
                    break;
            }
        }
    }
